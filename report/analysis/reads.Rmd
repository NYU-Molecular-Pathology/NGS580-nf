...
metrics_df <- do.call('rbind', lapply(X = samplesIDs, FUN = function(samplesID){
    ...
    dedup <- search_parse_file(pattern = dd_pattern, files = input_files, FUN = read.dedup.log, na = list())
    duplicates <- get_element_default(object = dedup, name = "Duplicates", default = NA)
    duplicates <- get_element_default(object = duplicates, name = 1, default = NA, is.index = TRUE)

    
    # stats from BWA alignment
    flagstat_pattern <- paste(samplesID, '.flagstat.txt', sep = '', collapse = '|')
    flagstat <- search_parse_file(pattern = flagstat_pattern, files = input_files, FUN = read.flagstat, na = list())
    mapped_reads <- get_element_default(object = flagstat, name = "MappedReads", default = NA)
    mapped_reads <- get_element_default(object = mapped_reads, name = 1, default = NA, is.index = TRUE)
    chimeric_reads <- get_element_default(object = flagstat, name = "ChimericReads", default = NA)
    chimeric_reads <- get_element_default(object = chimeric_reads, name = 1, default = NA, is.index = TRUE)
    
    # stats from deduplication
    flagstat_dd_pattern <- paste(samplesID, '.dd.flagstat.txt', sep = '', collapse = '|')
    flagstat_dd <- search_parse_file(pattern = flagstat_dd_pattern, files = input_files, FUN = read.flagstat, na = list())
    dedup_reads <- get_element_default(object = flagstat_dd, name = "MappedReads", default = NA)
    dedup_reads <- get_element_default(object = dedup_reads, name = 1, default = NA, is.index = TRUE)

...
Channel.fromPath( file(params.cnv_targetbed) ).set{ cnv_targetbed }
...
Channel.fromPath( file(params.samples_analysis_sheet) )
        ...
        .into { samples_pairs; samples_pairs2 ; samples_pairs3}
...
process sambamba_view_sort {
    ...
    output:
    set val(sampleID), file("${bam_file}") into samples_bam, samples_bam2, samples_bam3
...
samples_bam3.tap { samples_bam4; samples_bam5 }.subscribe { println "[samples_bam3] ${it}" }
samples_pairs3.tap { samples_pairs4; samples_pairs5 }.subscribe { println "[samples_pairs3] ${it}" }


samples_bam4.combine(samples_pairs4, by:0)
            .map {items ->
            def tumorID = items[0]
            def tumorBam = items[1]
            def normalID = items[2]
            return [ normalID, tumorID, tumorBam ]
        }
            .combine(samples_bam5, by:0)
            .map {normalID, tumorID, tumorBam, normalBam -> 
            return [ tumorID, tumorBam, normalID, normalBam]
            }
            .combine(ref_fasta4)
            .combine(cnv_targetbed)
            // .subscribe { println "[samples_bam4] ${it}" }
            .set { all_files_cnv }
...
process cnvkit {
    ...
    input:
    set val(tumorID), file(tumorBam), val(normalID), file(normalBam), file(ref_fasta), file(cnv_targetbed) from all_files_cnv
    ...
    """
    echo "[cnvkit] tumorID: ${tumorID}, tumorBam: ${tumorBam}, normalID: ${normalID}, normalBam: ${normalBam}, ref_fasta: ${ref_fasta},  cnv_targetbed: ${cnv_targetbed}"
    cnvkit.py batch "${tumorBam}" \
    --normal "${normalBam}" \
    --targets "${cnv_targetbed}" \
    --fasta "${ref_fasta}" \

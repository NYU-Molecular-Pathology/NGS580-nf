...
# data <- read.table("TS-17-22596-B18-498.cns",header = T,sep = "\t")
# gene <- read.table("TS-17-22596-B18-498.final.cns",header = T,sep = "\t")
# gene2 <- gene
# 
...
#                       nm = chrom_order)
# chrom_order <- factor(x = chrom_order, levels = rev(chrom_order))
# gene2[["chromosome"]] <- factor(x = gene2[["chromosome"]], 
#                                 levels = chrom_order)
# gene2[["gain_loss"]] <- factor(ifelse(gene2[["log2"]] > 0, "gain", "loss"))
# 
# 
# 
# cnvplot <- ggplot(data = gene2, 
#                   aes(x = start, 
#                       xend = end, 
#                       y = log2, 
#                       yend = log2, 
...
#           axis.ticks.x=element_blank(), 
#           panel.grid = element_blank()) + 
#     scale_y_continuous(limits = c( -max(gene2[["log2"]]) - 0.5, max(gene2[["log2"]]) + 0.5)) + 
#     theme(legend.position="none") + 
#     scale_color_manual(values = c("green", "red"))
# 
# 
...
# 
# htmlwidgets::saveWidget(as_widget(cnvplotly), file = "cnvplot.html", selfcontained = TRUE)
# 
# pdf(file = "cnvplot.pdf", width = 15, height = 8)
# print(cnvplot)
# dev.off()
# 
# # https://www.biostars.org/p/214100/ for other labeling ideas, also with ggrepel
# 
# save.image(file = "cnvplot.Rdata", compress = TRUE)
